...
int IOWarriorWriteToInterface (IOWarriorHIDDeviceInterface** inInterface, int inSize, void* inData)
{
    ...
    if (ioReturnValue != kIOReturnSuccess)
    {
        PrintErrMsgIfIOErr (ioReturnValue, "IOWarriorWriteToInterface: Could not write setReport on hid device interface");
...
int IOWarriorReadFromInterface (IOWarriorHIDDeviceInterface** inInterface, int inReportID, int inSize, void* outData)
{
    ...
    if (ioReturnValue != kIOReturnSuccess)
    {
        PrintErrMsgIfIOErr (ioReturnValue, "IOWarriorReadFromInterface: Could not write setReport on hid device interface");
        (*inInterface)->close (inInterface);
        return ioReturnValue;
    }
	else
	{
//	printf("IOWarriorReadFromInterface OK\n");
	}
    // close interface
    (*inInterface)->close (inInterface);
    return ioReturnValue;
