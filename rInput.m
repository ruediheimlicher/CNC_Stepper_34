...
- (NSArray*)ReadHexStringArray
{
   ...
   if (listNode->interfaceType == kIOWarrior24Interface0 ||
			...
			listNode->interfaceType == kIOWarrior24PVInterface0)
		{
			...
			if (result != 0)
			{
				NSRunAlertPanel (@"IOWarrior Error in 'ReadHexStringArray'", @"An error occured while trying to read from the selected IOWarrior device.", @"OK", nil, nil);
...
//	NSLog(@"InputAktion Zoom: %2.2f",[[[note userInfo] objectForKey:@"Zoom"]floatValue]);
	NSArray* InputArray=[self ReadHexStringArray];
...
- (void)trackRead:(NSTimer*) inTimer
{
        ...
        listNode->interfaceType == kIOWarrior40Interface0)
    {
      ...
      // NSLog(@"trackRead: reportSize: %d	strlen(buffer): %d",reportSize,strlen(buffer));
        ...
        if (result != 0)
        {
            NSRunAlertPanel (@"IOWarrior Error in 'trackRead'", @"An error occured while trying to read from the selected IOWarrior device.", @"OK", nil, nil);
            [self doRead:self]; // invalidates timer
            return ;
        }
